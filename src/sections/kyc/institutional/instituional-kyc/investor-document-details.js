...
export default function KYCMerchantDetails({
  ...
  const { kycSectionData, kycSectionLoading, kycSectionError, refreshKycSection } =
    useGetKycSection('investor_documents', '/company-kyc/company-details');
    ...
    try {
      ...
      const uploadedDocuments = documents
        .map((item) => {
          ...
          return {
            investorKycDocumentRequirementsId : item.documentId,
      ...
      const response = hasExistingDocs
        ? await axiosInstance.patch('/investor-profiles/kyc-upload-documents', payload)
        : await axiosInstance.post('/investor-profiles/kyc-upload-documents', payload);
